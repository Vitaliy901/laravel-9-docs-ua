...
За лаштунками метод `withoutOverlapping` використовує [кеш](cache.md) вашого додатка для отримання блокувань. Якщо необхідно, ви можете очистити ці блокування кешу за допомогою команди `schedule:clear-cache` Artisan. Зазвичай це необхідно, лише якщо завдання зависає через неочікувану проблему на сервері.
...
> **Warning**  
> Щоб скористатися цією функцією, ваш додаток має використовувати драйвер кешу `database`, `memcached`, `dynamodb`, or `redis` як драйвер кешу за замовчуванням. Крім того, всі сервери мають спілкуватися з одним центральним сервером кешу.
...
Отже, коли використовується планувальник Laravel, нам потрібно лише додати один запис конфігурації cron на наш сервер, який щохвилини запускає команду `schedule:run`. Якщо ви не знаєте, як додати записи cron на свій сервер, спробуйте скористатися службою, такою як [Laravel Forge](https://forge.laravel.com), яка може керувати записами cron за вас:
