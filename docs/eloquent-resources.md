...
На додаток до умови включення інформації про зв’язок у ваші відповіді ресурса, ви можете через умову включити дані з проміжних таблиць відношень «багато-до-багатьох» за допомогою метода `whenPivotLoaded`. Метод `whenPivotLoaded` приймає назву проиіжної таблиці як свій перший аргумент. Другим аргументом має бути замикання, яке повертає значення, яке буде повернуто, якщо в моделі доступна зведена інформація:
