...
> **Note**  
> Ви можите оголосити будь-які залежності, які вам потрібні, в сигнатурі метода `rules`. Вони будуть автоматично отримані за допомогою [контейнера служб](container.md) Laravel.
...
public function store(StorePostRequest $request)
{
	...
	// Отримати частину провалідованих вхідних даних...
...
> **Note**  
> Ви можете оголосити будь-які залежності які вам потрібні в сигнатурі методу `authorize`. Вони будуть автоматично витягнуті з [контейнера служб](container.md) Laravel.
...
Доступні обмеження:_min_width_, _max_width_, _min_height_, _max_height_, _width_, _height_, _ratio_.
...
> **Warning**  
> Валідатори `dns` і `spoof` потребують розширення `intl` PHP.
...
> **Warning**  
> Перераховані типи доступні тільки в [PHP 8.1+](https://www.php.net/manual/ru/language.enumerations.php).
...
> **Warning**  
> Це правило валідації не перевіряє те, що значення поля відноситься до типу `integer`, а виконує перевірку за правилами `FILTER_VALIDATE_INT` PHP. Якщо вам потрібно перевірити значення поля у якості числа, то використовуйте це правило у поєднанні з [правилом валідації `numeric`](#rule-numeric).
...
> **Warning**  
> Для використання правила `multiple_of` потрібне [розширення `bcmath` PHP.](https://www.php.net/manual/ru/book.bc.php).
...
> **Warning**  
> Під час використання шаблонів `regex` / `not_regex` може знадобитися вказати правила перевірки за допомогою масиву замість використання `|` роздільників, особливо якщо регулярний вираз містить символ `|`.
...
> **Warning**  
> Це правило було перейменовано в `current_password` з його наступним видаленням в Laravel 9. Замість цього правила використовуте правило [Current Password](#rule-current-password).
...
> **Note**  
> Якщо ви намагаєтеся перевірити поле, яке завжди має бути присутнім, але може бути порожнім, перегляньте цю [примітку щодо необов’язкових полів](#a-note-on-optional-fields).
...
> **Note**  
> Параметр `$input`, переданий вашому замиканню, буде екземпляром `Illuminate\Support\Fluent` і може використовуватися для доступу до ваших введених даних і файлів, що перевіряються.
...
// Потрібна хоча б одна літера...
Password::min(8)->letters()

// Вимагае принаймні одну велику та одну малу літери...
...
> **Note**  
> Починаючи з версії Laravel: 9.19.0, параметр `--invokable` доступний.
...
Після визначення правила ви можете приєднати його до валідатора, передавши екземпляр об’єкта правила з іншими правилами перевірки.Ви можете викликати помічника `trans` в методі `message`, якщо хочете повернути повідомлення про помилку з файлів перекладу:
...
> **Warning**  
> Неявне правило лише _передбачає_, що атрибут є обов'язковим до валідації. Насправді, вирішувати тільки вам, порожній або відсутній атрибут вважатиметься невалідним.
