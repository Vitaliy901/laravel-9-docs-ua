...
{
	/**
	 * Застосуйте область запита до заданого конструктора запитів Eloquent.
	 *
...
Якщо ви хочете видалити глобальну область для певного запита, ви можете використати метод `withoutGlobalScope`. Цей метод приймає назву класу глобальної області як єдиний аргумент:
...
Локальні області дозволяють визначати загальні набори обмежень запитів, які можна легко повторно використовувати у своему додатку. Наприклад, часте отримання «популярних» користувачів. Щоб визначити область запита, додайте префікс `scope` до моделі Eloquent.
...
class User extends Model
{
	...
	/**
	 * Область запиту містить лише активних користувачів.
	 *
	 * @param  \Illuminate\Database\Eloquent\Builder  $query
	 * @return void
	 */
