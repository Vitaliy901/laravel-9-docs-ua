...
> **Note**  
> Перш ніж почати, обов’язково налаштуйте з’єднання з базою даних у конфігураційному файлі додатку `config/database.php`. Для отримання додаткової інформації про налаштування бази даних перегляньте документацію щодо [налаштування бази даних](database.md#configuration).
...
За замовчуванням усі моделі Eloquent використовуватимуть стандартне підключення до бази даних, налаштоване для вашого додатка. Якщо ви хочете вказати інше з’єднання, яке слід використовувати під час взаємодії з певною моделлю, вам слід визначити властивість `$connection` у моделі:
