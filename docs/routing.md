...
> {tip} При визначенні кількох маршрутів, які мають один і той самий URI, маршрути, що використовують методи `get`, `post`, `put`, `patch`, `delete`, і `options`, повинні бути визначені перед маршрутами, які використовують методи `any`, `match`, і `redirect`. Це гарантує, що вхідний запит узгоджується з правильним маршрутом.
...
> {note} Під час використання параметрів маршрута в маршрутах перенаправлення такі параметри зарезервовані Laravel і не можуть бути використані: `destination` та `status`.
...
> {note} При використанні параметрів маршрута у маршрутах візуалізації, наступні параметри зарезервовані Laravel і не можуть бути використані: `data`, `status`, та `headers`.
...
> {note} Визначення похилого слеша підтримується лише в останньому сегменті маршруту.
...
```
> {note} Назви маршрутів завжди мають бути унікальними.
...
// /user/1/profile
```
> {tip} Іноді вам може знадобитися вказати значення за замовчуванням для параметрів URL-адреси на рівні запиту, як-от поточна мова. Щоб досягти цього, ви можете використати метод [`URL::defaults`](urls.md#default-values).
...
> {note} Щоб забезпечити доступність маршрутів піддоменів, необхідно зареєструвати маршрути піддоменів перед реєстрацією маршрутів корінного домену. Це запобігає перезапису маршрутами корінного домену маршрутів піддоменів, які мають однаковий шлях URI.
...
```
Оскільки змінна `$user` зазначена як модель `App\Models\User` Eloquent, а назва змінної відповідає сегменту URI `{user}`, Laravel автоматично введе екземпляр моделі, ID якої відповідає значенню з URI запиту. Якщо відповідний екземпляр моделі не знайдено в базі даних, автоматично буде згенеровано HTTP-відповідь 404.
...
Як правило, неявне прив’язування моделі не відновлює моделі, які було [програмно видалено](eloquent#soft-deleting). Однак ви можете вказати неявному зв’язуванню отримати ці моделі, приєднавши метод `withTrashed` до визначення вашого маршруту:
...
Ви не зобов’язані використовувати неявне прив'язування моделі на основі конвенцій Laravel, щоб використовувати зв’язування моделі. Ви також можете явно визначити, як параметри маршруту відповідають моделям. Щоб зареєструвати явне прив’язування, використовуйте метод `model` маршрутизатора, щоб указати клас для заданого параметра. Вам слід визначити явні прив’язки моделі на початку методу `boot` вашого класу `RouteServiceProvider`:
...
Обмежувачі можуть бути закріплені за маршрутами або групами маршрутів за допомогою [посередника](middleware) `throttle`. Посередник `throttle` приймає ім'я обмежувача, який ви хочете призначити маршруту:
...
```
Для зручності ви можете використовувати директиву шаблонизатора [Blade](blade) `@method` для створення поля введення `_method`:
...
> {tip} Для отримання додаткової інформації про CORS і заголовки CORS зверніться до [веб-документації MDN щодо CORS](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS#The_HTTP_response_headers).
