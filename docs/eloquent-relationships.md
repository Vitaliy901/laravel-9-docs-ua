...
Отже, ми можемо отримати доступ до моделі `Phone` з моделі `User`. Далі визначимо зв’язок на моделі `Phone`, який дозволить нам отримати доступ до користувача, якому належить телефон. Ми можемо визначити зворотний зв’язок `hasOne` за допомогою метода `belongsTo`:
...
Однак, якщо зовнішній ключ для вашого зв’язку не відповідає цим умовам, ви можете передати власне ім’я зовнішнього ключа другим аргументом метода `belongsTo`:
...
Метод `morphToMany` приймає назву пов’язаної моделі, а також «ім’я відношення». Базуючись на імені, яке ми визначели нашій проміжній таблиці, і ключах, які вона містить, ми називатимемо зв’язок «taggable»:
...
Далі в моделі `Tag` ви повинні визначити метод для кожної з її можливих батьківських моделей. Отже, у цьому прикладі ми визначимо метод `posts` і `videos`. Обидва ці методи мають повертати результат метода `morphedByMany`.
...
Ви також можете використовувати псевдонім результата підрахунка відношень, дозволивши декілька підрахунків для одного і того ж відношення:
...
Якщо ви об’єднуєте `withCount` із оператором `select`, переконайтеся, що ви викликаєте `withCount` після метода `select`:
...
Якщо ви поєднуєте ці агрегатні методи з оператором `select`, переконайтеся, що ви викликаєте агрегатні методи після метода `select`:
...
Цей приклад виконає один запит для отримання всіх книг у таблиці бази даних, а потім в циклі ще один запит для кожної книги, щоб отримати автора книги. Отже, якщо у нас є 25 книг, наведений вище код запустить 26 запитів: один для оригінальної книги та 25 додаткових запитів для отримання автора кожної книги.

На щастя, ми можемо використовувати нетерпляче завантаження, щоб скоротити цю операцію лише до двох запитів. Під час створення запита ви можете вказати, які відношенння слід завантажувати за допомогою метода `with`:
...
Іноді потрібно завантажити декілька різних відношень. Для цього просто передайте масив відношень методу `with`:
...
Якщо ви бажаєте завантажити відношення morphTo, Eloquent виконає декілька запитів, щоб отримати кожен тип пов’язаної моделі. Ви можете додати додаткові обмеження до кожного з цих запитів, використовуючи метод `constrain` поліморфного відношення `MorphTo`:
...
Ви можете налаштувати поведінку порушень відкладеного завантаження за допомогою метода `handleLazyLoadingViolationsUsing`. Наприклад, використовуючи цей метод, ви можете вказати лише реєструвати порушення відкладеного завантаження, а не переривати виконання додатка з винятками:
...
Якщо вам потрібно зберегти декілька зв'язаних моделей, ви можете використовувати метод `saveMany`:
