...
Ви можете використовувати метод `table`, наданий фасадом `DB`, щоб почати запит. Метод `table` повертає гнучкий екземпляр конструктора запитів для даної таблиці, дозволяючи вам зв’язати більше обмежень із запитом, а потім, нарешті, отримати результати запита за допомогою методу `get`:
...
Замість використання методу `DB::raw` ви також можете використовувати наведені нижче методи, щоб вставити необроблений вираз у різні частини вашого запита. **Пам’ятайте, що Laravel не може гарантувати, що будь-який запит, який використовує необроблені вирази, захищений від вразливості SQL-ін’єкції.**
...
Як і слід було очікувати, для групування результатів запита можна використовувати методи `groupBy` та `having`. Сигнатура методу `having` подібна до сигнатури метода `where`:
...
Іноді вам може знадобитися, щоб певні положення запита застосовувалися до запита на основі іншої умови. Наприклад, ви можете застосувати оператор `where`, лише якщо надане вхідне значення присутнє у вхідному HTTP-запиті. Ви можете зробити це за допомогою методу `when`:
