...
[Laravel Sanctum](https://github.com/laravel/sanctum) надає легку систему автентифікації для SPA (односторінкових програм), мобільних додатків і простих API на основі токенів. Sanctum дозволяє кожному користувачеві вашого додатка генерувати кілька токенів API для їхнього облікового запису. Цим токенам можуть бути надані повноваження/області, які визначають, які дії дозволено виконувати токенам.
...
Laravel Sanctum пропонує цей функціонал через збереження токенів API користувача в єдиній таблиці бази даних та автентифікуючи вхідні HTTP-запити через заголовок `Authorization`, який повинен містити дійсний токен API.

<a name="how-it-works-spa-authentication"></a>

#### Коротко про автентифікацію SPA
...
> **Note**  
> Найновіші версії Laravel вже містять Laravel Sanctum. Однак, якщо файл composer.json вашого додатка не містить `laravel/sanctum`, ви можете виконати інструкції встановлення, які наведені нижче.
...
Потім ви можете наказати Sanctum використовувати вашу спеціальну модель за допомогою метода `usePersonalAccessTokenModel`, наданого Sanctum. Як правило, ви повинні викликати цей метод у методі `boot` одного з провайдерів вашого додатка:
...
Sanctum дозволяє видавати токени API/особисті токени доступу, які можна використовувати для автентифікації запитів API до вашого додатка. Під час виконання запитів із використанням токенів API, токен має бути включений у заголовок `Authorization` як `Bearer` токен.
...
При опрацюванні вхідного запита, який автентифікований Sanctum, ви можете визначити, чи має токен зазначені повноваження, використовуючи метод `tokenCan`:
...
За замовчуванням токени Sanctum ніколи не закінчуються і можуть бути визнані недійсними лише шляхом [відкликання токена](#revoking-tokens). Однак, якщо ви хочете налаштувати термін дії API токенів вашого додатка, то ви можете зробити це за допомогою параметра `expiration`, визначеного в конфігураційному файлі `sanctum` вашого додатка. Цей параметр конфігурації визначає кількість хвилин, після яких виданий токен буде вважатися простроченим:
...
> **Warning**  
> Якщо ви отримуєте доступ до свого додатка через URL-адресу, яка містить порт (`127.0.0.1:8000`), переконайтеся, що номер порту включено в домен.
...
Якщо у вас виникли проблеми з автентифікацією вашого SPA, який виконується на окремому піддоміні, ви, ймовірно, неправильно налаштували параметри CORS (спільне використання ресурсів між різними джерелами) або cookie сесій.

Ви повинні переконатися, що конфігурація CORS додатка повертає заголовок `Access-Control-Allow-Credentials` зі значенням `true`. Цього можна досягти, встановивши для параметра `supports_credentials` значення `true` в конфігураційному файлі `config/cors.php` вашого додатка.

Крім того, ви повинні задіяти параметр `withCredentials` у глобальному екземплярі `axios` вашого додатка. Зазвичай це потрібно робити у файлі `resources/js/bootstrap.js`. Якщо ви на клієнтській стороні не використовуєте Axios для виконання HTTP-запитів, вам слід виконати аналогічне налаштування свого HTTP-клієнта:

```js
axios.defaults.withCredentials = true;
```

Нарешті, ви повинні переконатися, що конфігурація домена cookie сесії вашого додатка підтримує будь-який піддомен вашого кореневого домену. Ви можете зробити це, додавши домену префікс `.` у конфігураційному файлі `config/session.php` вашого додатка:
...
Для аутентифікації вашого SPA, сторінка «входу» повинна спочатку зробити запит до кінцевої точки `/sanctum/csrf-cookie` для ініціалізації захисту від CSRF для додатків:
...
Після того, як захист від CSRF був ініціалізований, ви повинні зробити `POST` запит до маршруту `/login` вашого додатка Laravel. Цей маршрут `/login` може бути [реалізований вручну](authentication.md#authenticating-users) або з використанням пакета безголової автентифікації, як [Laravel Fortify](fortify.md).

Якщо запит на вхід буде успішним, користувач буде автентифікований, і наступні запити до маршрутів вашого додатка будуть автоматично автентифікуватися через cookie сесії, які додаток Laravel надіслав клієнтській стороні. Крім того, оскільки ваш додаток вже зробив запит до маршруту `/sanctum/csrf-cookie`, то наступні запити повинні автоматично отримувати захист від CSRF, поки ваш HTTP-клієнт JavaScript надсилає значення `XSRF-TOKEN` cookie у заголовку `X-XSRF-TOKEN`.
...
Ви також можете використовувати токени Sanctum для автентифікації запитів вашого мобільного додатка до вашого API. Процес автентифікації запитів мобільних додатків подібний до автентифікації запитів сторонніх API; однак існують невеликі відмінності в тому, як ви будете випускати токени API.
...
Коли мобільний додаток використовує токен для надсилання запита API до вашого додатка, він повинна передавати токен у заголовку `Authorization` як токен `Bearer`.

> **Note**  
> Випускаючи токени для мобільного додатка, ви також можете вказати [повноваження токена](#token-abilities).
