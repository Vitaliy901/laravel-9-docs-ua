...
Sanctum також включає два посередника, які можна використовувати для перевірки автентифікації вхідного запиту за допомогою токена, якому надано певні повноваження. Щоб розпочати, додайте наступні посередники до властивості `$routeMiddleware` файлу `app/Http/Kernel.php` вашого додатку :
...
Посередник `abilities` може бути призначено маршруту для перевірки того, що токен вхідного запиту має всі перелічені повноваження:
...
Посередник `ability` може бути призначений маршруту для перевірки того, що токен вхідного запиту має _принаймні одне_ з перерахованих повноважень:
